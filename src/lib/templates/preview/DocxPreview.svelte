	...
	{#each pages as page, pi (pi)}
		<div class="docx-preview relative shadow-md" style={pageStyle}>
			{#if doc.header?.length}
				<div
					class="absolute"
					style="top:{doc.headerDistance}pt;left:{doc.pageMargins.left}pt;right:{doc.pageMargins.right}pt"
				>
			...
			{#if doc.footer?.length}
				<div
					class="absolute"
					style="bottom:{doc.footerDistance}pt;left:{doc.pageMargins.left}pt;right:{doc.pageMargins.right}pt"
				>
