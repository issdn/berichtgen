<script lang="ts">
	...
	let otpCooldownTimer: null | ReturnType<typeof setInterval> = null;
	...
	$effect(() => {
		...
		if (!email) return;
		void verifyOtpResource.execute({ email, token });
	});
	...
	const { enhance: mailInputEnhance, form: formData, submitting } = form;
