...
export function validateCanReport(
	...
	if (template!.user_id === userId) throwSvelteError(ETemplateReportError.CANNOT_REPORT_OWN);
...
export async function uploadTemplateFile(
	...
	const { error } = await supabaseAdmin.storage
		.from('templates')
		.upload(storagePath, params.data, { contentType: params.type, upsert: true });
...
export async function deleteTemplateFile(storagePath: string): Promise<void> {
	const { error } = await supabaseAdmin.storage.from('templates').remove([storagePath]);
	if (error) throwSvelteError(ECommonServerError.DATABASE_ERROR, error.message);
 ...
 */
export async function deleteTemplateReport(templateId: string, userId: string): Promise<void> {
	try {
		await db
...
export async function submitTemplateReport(
	...
	try {
		await db
			...
			.values({ template_id: templateId, reporter_user_id: userId, message: message ?? null })
			.execute();
	} catch (e) {
	...
	try {
		...
		if (fileResponse.ok) {
			...
			if (uploadError) Sentry.captureException(uploadError, { extra: { templateId } });
		}
	} catch (e) {
...
export async function markTemplateSafeById(templateId: string): Promise<void> {
	const { error } = await supabaseAdmin.storage.from('quarantine').remove([templateId]);
	...
	await db.deleteFrom('template_report').where('template_id', '=', templateId).execute();
}
