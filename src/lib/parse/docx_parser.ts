...
export class DOCXParser extends Parser {
	...
	async init(data: Uint8Array) {
		...
		const [hdrEntries, ftrEntries, rawRels] = await Promise.all([
			Promise.all(hdrNames.map(async (f): Promise<[string, string]> => [f.split('/').at(-1)!, await docx.files[f].async('string')])),
			Promise.all(ftrNames.map(async (f): Promise<[string, string]> => [f.split('/').at(-1)!, await docx.files[f].async('string')])),
			docx.files['word/_rels/document.xml.rels']?.async('string') ?? Promise.resolve('')
		]);
		...
		for (const rel of toArray(parsedRels?.Relationships?.Relationship)) {
			...
			if (r['@_Id'] && r['@_Target']) docRels.set(r['@_Id'], r['@_Target'].split('/').at(-1)!);
		}
	...
	parseAST(): DocxDocument {
		...
		const docBody = (
			(this.data.xml['w:document'] as Record<string, unknown>)?.['w:body'] as Record<string, unknown>
		) ?? {};
		...
		const orderedXml = orderedParser.parse(this.data.xmlStr) as Array<Record<string, unknown>>;
		const docEntry = orderedXml.find((e) => 'w:document' in e);
		const docEntries = (docEntry?.['w:document'] as Array<Record<string, unknown>>) ?? [];
		const bodyEntry = docEntries.find((e) => 'w:body' in e);
		const bodyChildren = (bodyEntry?.['w:body'] as Array<Record<string, unknown>>) ?? [];
		...
		const pageWidth  = pgSz?.['@_w:w'] !== undefined ? Number(pgSz['@_w:w']) / 20 : 612;
		const pageHeight = pgSz?.['@_w:h'] !== undefined ? Number(pgSz['@_w:h']) / 20 : 792;
		const headerDistance = pgMar?.['@_w:header'] !== undefined ? Number(pgMar['@_w:header']) / 20 : 35.4;
		const footerDistance = pgMar?.['@_w:footer'] !== undefined ? Number(pgMar['@_w:footer']) / 20 : 35.4;
		const pageMargins = {
			top:    pgMar?.['@_w:top']    !== undefined ? Number(pgMar['@_w:top'])    / 20 : 72,
			right:  pgMar?.['@_w:right']  !== undefined ? Number(pgMar['@_w:right'])  / 20 : 90,
			bottom: pgMar?.['@_w:bottom'] !== undefined ? Number(pgMar['@_w:bottom']) / 20 : 72,
			left:   pgMar?.['@_w:left']   !== undefined ? Number(pgMar['@_w:left'])   / 20 : 90
		};

		// Default font size from w:docDefaults rPr (half-points → pt).
		const defaultRpr = this.data.styleRprs.get('Normal') ?? {};
		const defaultFontSize = numVal(defaultRpr['w:sz']) != null
			? numVal(defaultRpr['w:sz'])! / 2
			: 11;

		// Resolve default header/footer via sectPr > w:headerReference / w:footerReference.
		const hfParser = new XMLParser({ ignoreAttributes: false, attributeNamePrefix: '@_', trimValues: false });
		const parseHF = (xmlStr: string | undefined, rootTag: string): DocxParagraph[] => {
			if (!xmlStr) return [];
			const root = hfParser.parse(xmlStr);
			const hf = root[rootTag] as Record<string, unknown> | undefined;
			if (!hf) return [];
			return toArray(hf['w:p']).map(p => this.mapParagraph(p as Record<string, unknown>, fontsUsed));
		};
		const resolveHF = (refs: Record<string, string>[], xmlMap: Map<string, string>): string | undefined => {
			const defaultRef = refs.find(r => r['@_w:type'] === 'default') ?? refs[0];
			const rId = defaultRef?.['@_r:id'];
			const file = rId ? this.data!.docRels.get(rId) : undefined;
			return file ? xmlMap.get(file) : undefined;
		};
		const hdrRefs = toArray(sectPr?.['w:headerReference']) as Record<string, string>[];
		const ftrRefs = toArray(sectPr?.['w:footerReference']) as Record<string, string>[];
	...
	private mapParagraph(p: Record<string, unknown>, fontsUsed: Set<string> = new Set()): DocxParagraph {
		const pPr = p['w:pPr'] as Record<string, unknown> | undefined;
		const style = (pPr?.['w:pStyle'] as Record<string, string> | undefined)?.['@_w:val'];
		...
		const alignment = (effectivePpr['w:jc'] as Record<string, string> | undefined)?.['@_w:val'];

		// Spacing (w:spacing attributes are directly on the element, not nested).
		const spacingNode = effectivePpr['w:spacing'] as Record<string, unknown> | undefined;
		const spacingBefore = spacingNode?.['@_w:before'] !== undefined
			? Number(spacingNode['@_w:before']) / 20
			: undefined;
		const spacingAfter = spacingNode?.['@_w:after'] !== undefined
		...
		if (spacingNode?.['@_w:line'] !== undefined) {
			...
			const rule: 'auto' | 'exact' | 'atLeast' =
				rawRule === 'exact' ? 'exact' : rawRule === 'atLeast' ? 'atLeast' : 'auto';
			lineSpacing = { value: Number(spacingNode['@_w:line']), rule };
		}

		// Left indent.
		const indentNode = effectivePpr['w:ind'] as Record<string, unknown> | undefined;
		const indent = indentNode?.['@_w:left'] !== undefined
		...
		for (const r of toArray(p['w:r'])) {
			...
			const fontFamily = (effectiveRpr['w:rFonts'] as Record<string, string> | undefined)?.['@_w:ascii'];
			...
			runs.push({
				...
				color: (effectiveRpr['w:color'] as Record<string, string> | undefined)?.['@_w:val'],
				size: numVal(effectiveRpr['w:sz']),
				fontFamily: fontFamily || undefined
			} satisfies DocxRun);
		}

		return { type: 'paragraph', style, alignment, numPr, spacingBefore, spacingAfter, indent, lineSpacing, runs };
	}

	private mapTable(t: Record<string, unknown>, fontsUsed: Set<string> = new Set()): DocxTable {
		...
		const colWidths = tblGrid
			? toArray(tblGrid['w:gridCol']).map((col) =>
					Number((col as Record<string, unknown>)['@_w:w'] ?? 0) / 20
				)
			: undefined;
		...
		for (const tr of toArray(t['w:tr'])) {
			...
			const height = trHeight?.['@_w:val'] !== undefined
			...
			for (const tc of toArray(trNode['w:tc'])) {
				...
				const colspan = numVal(tcPr?.['w:gridSpan'] as Record<string, unknown> | undefined);
				const vMerge = tcPr?.['w:vMerge'] as Record<string, unknown> | undefined;
				...
				const vAlignRaw = (tcPr?.['w:vAlign'] as Record<string, unknown> | undefined)?.['@_w:val'];
				const vAlign = vAlignRaw === 'center' ? 'center'
					: vAlignRaw === 'bottom' ? 'bottom'
					: vAlignRaw === 'top' ? 'top'
					: undefined;
	...
	private async parseStyles(
		docx: JSZip,
		parser: XMLParser
	): Promise<{ styleRprs: Map<string, Record<string, unknown>>; stylePprs: Map<string, Record<string, unknown>> }> {
		...
		const defaultRpr = ((docDefaults?.['w:rPrDefault'] as Record<string, unknown>)?.['w:rPr'] ?? {}) as Record<string, unknown>;
		const defaultPpr = ((docDefaults?.['w:pPrDefault'] as Record<string, unknown>)?.['w:pPr'] ?? {}) as Record<string, unknown>;

		type RawStyle = { rPr: Record<string, unknown>; pPr: Record<string, unknown>; basedOn?: string };
		...
		for (const style of toArray(stylesRoot['w:style'])) {
			...
			rawStyles.set(styleId, {
				...
				basedOn: (s['w:basedOn'] as Record<string, string> | undefined)?.['@_w:val']
			});
		}
		...
		const resolveRpr = (styleId: string, stack = new Set<string>()): Record<string, unknown> => {
		...
		const resolvePpr = (styleId: string, stack = new Set<string>()): Record<string, unknown> => {
...
function hasPageBreak(p: Record<string, unknown>): boolean {
	...
	return toArray(p['w:r']).some((r) => {
		...
		if (brs.some((br) => (br as Record<string, unknown>)['@_w:type'] === 'page')) return true;
