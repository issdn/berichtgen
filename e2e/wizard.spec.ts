...
test.describe('Initial UI State', () => {
	...
	test('dropzone is visible with correct placeholder text', async ({
		page
	}) => {
		await expect(page.getByTestId('dropzone')).toBeVisible();
		await expect(page.getByTestId('dropzone')).toContainText(
	...
	test('completion button is disabled before any results are ready', async ({
		page
	}) => {
...
test.describe('JSON File Processing (Happy Path)', () => {
	test('file card appears after dropping a JSON file', async ({ page }) => {
		...
		await expect(page.getByTestId('wizard-file')).toBeVisible({
			timeout: 5000
		});
		await expect(page.getByTestId('wizard-file-name')).toContainText(
			'bericht.json'
		);
	});

	test('empty state is hidden once a file is being processed', async ({
		page
	}) => {
		await page.goto('/board');
	...
	test('JSON file reaches DONE state without any user interaction', async ({
		page
	}) => {
		await page.goto('/board');
		await dropJsonFile(page);
		...
		await expect(page.getByTestId('wizard-file-status')).toContainText(
			'Fertig',
			{
				timeout: 15000
			}
	...
	test('completion button becomes enabled after a JSON file finishes', async ({
		page
	}) => {
		await page.goto('/board');
		await dropJsonFile(page);
		...
		await expect(
			page.getByTestId('wizard-completion-button')
		).not.toBeDisabled();
	});
});
...
test.describe('Completion Dialog', () => {
	...
	test('clicking the completion button opens the download dialog', async ({
		page
	}) => {
...
test.describe('Multiple File Processing', () => {
	test('processes multiple JSON files concurrently and enables completion button when all finish', async ({
		...
		await expect(page.getByTestId('wizard-file-status').nth(0)).toContainText(
			'Fertig',
			{
				timeout: 15000
			}
		);
		await expect(page.getByTestId('wizard-file-status').nth(1)).toContainText(
			'Fertig',
			{
				timeout: 15000
			}
